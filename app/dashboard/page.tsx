...
const inputStyle: React.CSSProperties = {
  ...
  outline:"none", width:"90px", fontFamily:"inherit",
}
...
function KpiCard({ label, val, color, pctVal }: { label:string; val:number|string; color:string; pctVal?:number }) {
  return (
    <div style={{ background:"linear-gradient(135deg,#0f172a,#0d1424)", border:`1px solid ${color}30`, borderRadius:10, padding:"14px 16px" }}>
      ...
      <div style={{ display:"flex", alignItems:"baseline", gap:8, marginTop:4 }}>
        ...
        {pctVal !== undefined && <span style={{ fontSize:11, color, opacity:0.7 }}>{pctVal}%</span>}
      </div>
      {pctVal !== undefined && (
...
function TipoCard({ tipo, list, accent }: { tipo:string; list:Ambulancia[]; accent:string }) {
  ...
  return (
    <div style={{ background:"linear-gradient(135deg,#0f172a,#0d1424)", border:`1px solid ${accent}25`, borderRadius:10, padding:"14px 16px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:6 }}>
        {[{label:"Operativas",val:op,color:"#4ade80"},{label:"Mtto",val:mtto,color:"#fbbf24"},{label:"No op.",val:fuera,color:"#f87171"}].map(k => (
...
export default function Dashboard() {
  ...
  async function guardarEdicion(id: string) {
    await supabase.from("ambulancias").update({
      ...
      placa: editData.placa, marca: editData.marca, tipo: editData.tipo,
    }).eq("id", id)
    ...
    <div style={{
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:24 }}>
        <div>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <h1 style={{ margin:0, fontSize:20, fontWeight:800, letterSpacing:"0.05em", color:"#e2e8f0" }}>CENTRO DE CONTROL DE AMBULANCIAS</h1>
          </div>
          <p style={{ margin:"4px 0 0 32px", fontSize:11, color:"#475569", letterSpacing:"0.08em" }}>DIRECCIÓN PROVINCIAL DE SALUD DEL GUAYAS</p>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <span style={{ fontSize:11, color:"#64748b" }}><b style={{ color:"#e2e8f0" }}>{nombre}</b> | {rol}</span>
          <button onClick={cerrarSesion} style={{ background:"rgba(239,68,68,0.15)", border:"1px solid rgba(239,68,68,0.35)", color:"#f87171", padding:"7px 14px", borderRadius:6, fontSize:10, fontWeight:700, cursor:"pointer" }}>🔐 Salir</button>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:20, flexWrap:"wrap" }}>
        {[
          ...
          <button key={b.label} onClick={() => router.push(b.path)} style={{ background:b.color, color:"white", border:"none", padding:"9px 16px", borderRadius:7, fontSize:11, fontWeight:700, cursor:"pointer" }}>{b.label}</button>
        ))}
      </div>

      {mttoVencido.length > 0 && (
        <div style={{ background:"rgba(127,29,29,0.5)", border:"1px solid rgba(239,68,68,0.4)", borderRadius:8, padding:"10px 16px", marginBottom:8, fontSize:12 }}>
          🚨 <b>Mantenimiento vencido:</b> {mttoVencido.map(a=>a.codigo_operativo).join(", ")}
        </div>
      )}
      {mttoProximo.length > 0 && (
        <div style={{ background:"rgba(120,53,15,0.5)", border:"1px solid rgba(251,191,36,0.4)", borderRadius:8, padding:"10px 16px", marginBottom:20, fontSize:12 }}>
          ⚠️ <b>Próximo mantenimiento:</b> {mttoProximo.map(a=>a.codigo_operativo).join(", ")}
        </div>
      )}
      {alertas.length > 0 && (
        <div style={{ background:"rgba(127,29,29,0.4)", border:"1px solid rgba(239,68,68,0.3)", borderRadius:8, padding:"10px 16px", marginBottom:20, fontSize:12 }}>
          🔴 <b>{alertas.length} falla(s) crítica(s) abiertas</b>
        </div>
      )}

      <div style={{ marginBottom:6 }}><span style={{ fontSize:10, color:"#475569", letterSpacing:"0.12em", fontWeight:700 }}>▸ FLOTA TOTAL</span></div>
      ...
      <div style={{ marginBottom:6 }}><span style={{ fontSize:10, color:"#475569", letterSpacing:"0.12em", fontWeight:700 }}>▸ POR TIPO DE UNIDAD</span></div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
        <span style={{ fontSize:10, color:"#475569", letterSpacing:"0.12em", fontWeight:700 }}>▸ FLOTA — {visible.length} unidades</span>
        <div style={{ display:"flex", gap:6 }}>
          {(["TODOS","ALFA","BRAVO"] as const).map(f => (
            <button key={f} onClick={() => setFiltro(f)} style={{ background:filtro===f?"#1e3a5f":"rgba(255,255,255,0.04)", border:`1px solid ${filtro===f?"#38bdf8":"rgba(255,255,255,0.08)"}`, color:filtro===f?"#38bdf8":"#64748b", padding:"6px 14px", borderRadius:6, fontSize:10, fontWeight:700, cursor:"pointer" }}>{f}</button>
          ))}
        </div>
      </div>
          ...
          <thead>
            <tr style={{ background:"rgba(255,255,255,0.03)", borderBottom:"1px solid rgba(255,255,255,0.07)" }}>
              {["Estado","Código","Placa","Marca","Tipo","Base Operativa","KM Actual","Horas","Acciones"].map(h => (
                <th key={h} style={{ padding:"11px 14px", textAlign:"left", fontSize:9, color:"#475569", letterSpacing:"0.1em", fontWeight:700 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
              ...
              return (
                ...
                >
                  <td style={{ padding:"10px 14px" }}>
                    <span style={{ background:c.bg, border:`1px solid ${c.border}`, color:c.text, fontSize:9, fontWeight:700, padding:"3px 9px", borderRadius:4, letterSpacing:"0.05em", whiteSpace:"nowrap" }}>{a.estado.toUpperCase()}</span>
                  </td>
                  <td style={{ padding:"10px 14px" }}>
                    {isEdit ? <input value={editData.codigo_operativo||""} style={inputStyle} onChange={e=>setEditData({...editData,codigo_operativo:e.target.value})}/> : <span style={{ fontWeight:800, color:"#e2e8f0" }}>{a.codigo_operativo}</span>}
                  </td>
                  <td style={{ padding:"10px 14px" }}>
                  ...
                  <td style={{ padding:"10px 14px" }}>
                    {isEdit
                      ...
                      : <span style={{ background:a.tipo==="ALFA"?"rgba(56,189,248,0.1)":"rgba(167,139,250,0.1)", color:a.tipo==="ALFA"?"#38bdf8":"#a78bfa", border:`1px solid ${a.tipo==="ALFA"?"rgba(56,189,248,0.25)":"rgba(167,139,250,0.25)"}`, fontSize:9, fontWeight:800, padding:"2px 8px", borderRadius:4 }}>{a.tipo}</span>}
                  </td>
                  <td style={{ padding:"10px 14px", color:"#64748b", fontSize:10 }}>{a.base_operativa||"—"}</td>
                  <td style={{ padding:"10px 14px" }}>
                    <span style={{ color:kmAlerta?"#f87171":kmProximo?"#fbbf24":"#94a3b8" }}>{a.kilometraje_actual?.toLocaleString()}</span>
                  ...
                  <td style={{ padding:"10px 14px" }}>
                    {isEdit ? (
                      <div style={{ display:"flex", gap:6 }}>
                        <button onClick={() => guardarEdicion(a.id)} style={{ background:"rgba(74,222,128,0.15)", border:"1px solid rgba(74,222,128,0.4)", color:"#4ade80", padding:"6px 14px", borderRadius:6, fontSize:10, fontWeight:700, cursor:"pointer" }}>💾 Guardar</button>
                        <button onClick={() => setEditando(null)} style={{ background:"rgba(248,113,113,0.1)", border:"1px solid rgba(248,113,113,0.3)", color:"#f87171", padding:"6px 12px", borderRadius:6, fontSize:10, fontWeight:700, cursor:"pointer" }}>✕ Cancelar</button>
                      </div>
                    ) : (
                      <div style={{ display:"flex", gap:5, alignItems:"center" }}>
                        {/* BOTÓN FUSIONADO — abre drawer */}
                        <button
                          onClick={() => { setDrawerAmbId(a.id); setDrawerOpen(true) }}
                          style={{
                            background:"linear-gradient(135deg,rgba(34,211,238,0.18),rgba(56,189,248,0.1))",
                            border:"1px solid rgba(34,211,238,0.45)",
                            color:"#22d3ee", padding:"6px 13px", borderRadius:6,
                            fontSize:10, fontWeight:800, cursor:"pointer", whiteSpace:"nowrap",
                            boxShadow:"0 0 10px rgba(34,211,238,0.12)", letterSpacing:"0.03em",
                          }}>📋 Ficha</button>
                        <button onClick={() => { setEditando(a.id); setEditData(a) }} style={{ background:"rgba(251,191,36,0.12)", border:"1px solid rgba(251,191,36,0.3)", color:"#fbbf24", padding:"6px 11px", borderRadius:6, fontSize:10, fontWeight:700, cursor:"pointer" }}>✏️ Editar</button>
                        <button onClick={() => eliminarAmbulancia(a.id)} style={{ background:"rgba(239,68,68,0.12)", border:"1px solid rgba(239,68,68,0.3)", color:"#f87171", padding:"6px 10px", borderRadius:6, fontSize:12, cursor:"pointer" }}>🗑</button>
                      </div>
                    )}
                  </td>
                </tr>
      ...
      <div style={{ marginTop:16, display:"flex", justifyContent:"space-between", fontSize:9, color:"#1e293b" }}>
