...
/* ── COLORES CORREGIDOS — más oscuros y legibles ── */
  ...
  return (
    <div style={{ background:'linear-gradient(135deg,rgba(15,23,42,0.95),rgba(13,20,36,0.95))', border:`1px solid ${accent}25`, borderRadius:12, padding:'12px 13px' }}>
      ...
      <div style={{ display:'flex', gap:1, height:4, borderRadius:2, overflow:'hidden' }}>
        <div style={{ flex:op, background:'#22c55e' }}/><div style={{ flex:mt, background:'#d97706' }}/><div style={{ flex:fu, background:'#dc2626' }}/>
      </div>
    </div>
  )
}
...
function BuscadorUnidad({
  ...
  const kmAlert = resultado && resultado.kilometraje_actual >= resultado.kilometraje_mtto
  ...
  const sty = {
    ...
    label: { margin:0, fontSize:7, color:"#1e293b", letterSpacing:"0.1em", fontWeight:700 } as React.CSSProperties,
    val:   { margin:"3px 0 0", fontSize:11, fontWeight:700 } as React.CSSProperties,
  }
    ...
    <div style={{
        ...
        <div style={{
          ...
          <div style={{ padding:'14px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
            {[
              ...
              { l:"HORAS FUERA", v:resultado.horas+"h",                                  c:"#64748b"                     },
              ...
              <div key={f.l} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:8, padding:'8px 10px' }}>
                <p style={sty.label}>{f.l}</p>
                <p style={{ ...sty.val, color:f.c, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{f.v}</p>
              </div>
            ))}
          </div>
          ...
          <div style={{ padding:'0 14px 14px', display:'grid', gridTemplateColumns:'1fr 1fr 42px', gap:7 }}>
            ...
            <button
              onClick={limpiar}
              style={{ background:'rgba(220,38,38,0.1)', border:'1px solid rgba(220,38,38,0.25)', color:'#dc2626', padding:'10px', borderRadius:8, fontSize:12, cursor:'pointer' }}>
              ✕
            </button>
          </div>
        </div>
      ...
      {buscado && !resultado && (
        <div style={{ marginTop:14, padding:'20px', textAlign:'center', border:'1px dashed rgba(255,255,255,0.07)', borderRadius:10 }}>
          ...
          <p style={{ margin:'4px 0 0', fontSize:8, color:'#334155' }}>Intenta con código operativo (GA-01) o placa (MEA1891)</p>
        </div>
      )}
    </div>
  )
...
export default function Dashboard() {
  ...
  async function guardarEdicion(id: string) {
    await supabase.from("ambulancias").update({
      codigo_operativo:editData.codigo_operativo, placa:editData.placa,
      marca:editData.marca, tipo:editData.tipo,
    }).eq("id",id)
    setEditando(null); cargar()
  }

  function abrirEdicion(a: Ambulancia) {
    setEditando(a.id); setEditData(a)
  }
      ...
      <div style={{ position:'sticky', top:0, zIndex:20, background:'rgba(5,11,21,0.96)', backdropFilter:'blur(12px)', borderBottom:'1px solid rgba(255,255,255,0.07)', padding:'12px 16px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:7 }}>
            <span style={{ fontSize:10, color:'#64748b' }}><b style={{ color:'#cbd5e1' }}>{nombre}</b></span>
      ...
      <div style={{ padding:'14px 16px 30px', position:'relative', zIndex:1 }}>
        ...
        {mttoVencido.length > 0 && (
          <div style={{ background:'linear-gradient(135deg,rgba(127,29,29,0.5),rgba(127,29,29,0.25))', border:'1px solid rgba(220,38,38,0.35)', borderRadius:10, padding:'9px 13px', marginBottom:8, fontSize:10 }}>
            🚨 <b style={{ color:'#fca5a5' }}>Mtto vencido:</b> <span style={{ color:'#cbd5e1' }}>{mttoVencido.map(a=>a.codigo_operativo).join(", ")}</span>
          </div>
        )}
        {mttoProximo.length > 0 && (
          <div style={{ background:'linear-gradient(135deg,rgba(120,53,15,0.5),rgba(120,53,15,0.25))', border:'1px solid rgba(217,119,6,0.35)', borderRadius:10, padding:'9px 13px', marginBottom:8, fontSize:10 }}>
            ⚠️ <b style={{ color:'#fde68a' }}>Próximo mtto:</b> <span style={{ color:'#cbd5e1' }}>{mttoProximo.map(a=>a.codigo_operativo).join(", ")}</span>
          </div>
        )}
        {alertas.length > 0 && (
              ...
              {visible.map((a, i) => {
                ...
                return (
                  <tr key={a.id}
                    ...
                    style={{ borderBottom:'1px solid rgba(255,255,255,0.05)', background:isEdit?"rgba(56,189,248,0.05)":isHover?"rgba(255,255,255,0.03)":i%2===0?"transparent":"rgba(255,255,255,0.015)", transition:'background 0.15s', borderLeft:kmAlerta?"2px solid #dc2626":kmProximo?"2px solid #d97706":"2px solid transparent" }}
                  >
                    ...
                    <td style={{ padding:'9px 10px', color:'#94a3b8', fontSize:9, maxWidth:90, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{a.base_operativa||"—"}</td>
                    <td style={{ padding:'9px 10px' }}>
          ...
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.88)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:50, padding:16, backdropFilter:'blur(4px)' }}>
            <div style={{ background:'linear-gradient(135deg,#0f172a,#0b1120)', border:'1px solid rgba(34,211,238,0.18)', borderRadius:16, padding:20, width:'100%', maxWidth:360 }}>
              ...
              <div style={{marginBottom:14}}>
                ...
                <select value={editData.tipo||""} onChange={e=>setEditData({...editData,tipo:e.target.value})} style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.1)',color:'#f1f5f9',padding:'9px 12px',borderRadius:8,fontSize:11,outline:'none',width:'100%',fontFamily:"'Space Mono','Courier New',monospace"}}>
